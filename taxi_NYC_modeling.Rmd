...
In this document, we focus on applying the insights obtained from the previous analytic document by transforming data in a better shape, and creating a simple predicting model from that cleaned dataset.

In this case, we take a subsample of 1 millions rows from each monthly dataset in order to speed up computations this preliminary analysis. In next steps, we can further investigate the datase using a big data platform, such as Spark.
